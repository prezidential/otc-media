...
import type { IntegrationQueryResult, IntegrationToolContext } from "./types";
...
export async function runIntegrationQuery(
  ...
  const agentTools: AgentTool[] = plugin.tools.map((t) => ({
    ...
    execute: (params) => plugin.callTool(t.name, params, ctx),
  }));
...
export async function callIntegrationTool(
  ...
  try {
    const data = await plugin.callTool(toolName, params, ctx);
    return { ok: true, data };
  } catch (err) {
