...
Editorial Steering Inputs (adjust Opening Hook, Deep Dive, and The Last Word to match these):
{{STEERING_BLOCK}}

{{ANGLE_BLOCK}}

Approved leads (use exactly 3 for Fresh Signals — no more):
{{LEADS_BLOCK}}
...
- Include one anchor sentence that states the thesis as a direct claim. Do not bury it in the middle.
...
3) Fresh Signals (use exactly 3 leads from above — no more). Structure the Fresh Signals section in TWO parts inside the same markdown section:
...
- Write in first person. Use "I" to state positions and frame arguments.
- Keep paragraphs 1–2 sentences max.
- Maintain escalation.
- End with strategic clarity, not summary.
5) The Last Word (one closing paragraph, 2–3 sentences, first person, practitioner voice — use the Last Word seed from the angle as the basis)
...
Output valid JSON only. No markdown code fence. No commentary. Use this exact shape (all strings; hook_paragraphs is an array of strings):
{
  ...
  "deep_dive": "Full Deep Dive prose (600-900 words). First person. Use **bold** for 3 declarative statements.",
  "last_word": "One direct closing paragraph, 2-3 sentences, first person. State the practitioner takeaway."
}
...
- Deep Dive: write in first person; use "I" to state positions.
- The Last Word is one direct paragraph (2-3 sentences, first person) — not a list.
