  ...
  return [
    {
      ...
      execute: async () => {
        ...
        if (!sources || sources.length === 0) {
          return { sources: [], message: "No approved sources found. Ask the user to add sources on the Research Setup tab." };
        }
      ...
      execute: async (params) => {
        ...
        for (const source of sources) {
          ...
          try {
            ...
            // Update last_ingested_at on the research_sources row
            await supabase
              .from("research_sources")
