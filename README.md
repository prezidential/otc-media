...
# Required for Beehiiv MCP OAuth — app-layer key that encrypts stored tokens.
# Any non-empty secret (a random 32+ byte value is best): openssl rand -base64 32
BEEHIIV_TOKEN_ENC_KEY=your-random-token-encryption-key
...
- **Beehiiv MCP OAuth** — the Beehiiv MCP server uses OAuth 2.1 (DCR + PKCE), so a static API key is rejected (401). Apply `lib/supabase/schema-beehiiv-oauth.sql` in the Supabase SQL editor (creates `beehiiv_oauth_connections` + the `mcp_oauth_clients` DCR registry; idempotent). Set a random `BEEHIIV_TOKEN_ENC_KEY` (app-layer AES-256-GCM key for tokens at rest — e.g. `openssl rand -base64 32`), plus `BEEHIIV_MCP_SERVER_URL` + `BEEHIIV_PUBLICATION_ID`. Then click **Connect Beehiiv** on the Analytics dashboard to authorize; tokens are encrypted in the app layer and auto-refreshed.
